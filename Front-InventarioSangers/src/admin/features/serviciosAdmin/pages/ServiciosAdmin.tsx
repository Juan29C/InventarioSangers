...
export default function ServiciosAdmin() {
    ...
    const allData = [
        {
            id: 1,
            mes: "Enero",
            nombreEmpresa: "Empresa ABC S.A.C.",
            telefono: "+51 999 888 777",
            rucDni: "20123456789",
            capacidad: 100,
            proceso: "PENDIENTE",
            tecnicoResponsable: "Juan Pérez",
            fechaServicioRealizado: "2025-01-15",
            fechaProximoServicio: "2025-07-15",
            montoPagadoPrimerServicio: 150.00,
            montoPagadoSegundoServicio: 0,
            tipoServicio: "Mantenimiento"
        },
        {
            id: 2,
            mes: "Febrero",
            nombreEmpresa: "Comercial XYZ E.I.R.L.",
            telefono: "+51 987 654 321",
            rucDni: "10987654321",
            capacidad: 50,
            proceso: "REALIZADO",
            tecnicoResponsable: "María García",
            fechaServicioRealizado: "2025-02-10",
            fechaProximoServicio: "2025-08-10",
            montoPagadoPrimerServicio: 200.00,
            montoPagadoSegundoServicio: 200.00,
            tipoServicio: "Instalación"
        },
        {
            id: 3,
            mes: "Marzo",
            nombreEmpresa: "Industrias DEF S.A.",
            ...
            fechaProximoServicio: "2025-09-20",
            montoPagadoPrimerServicio: 0,
            montoPagadoSegundoServicio: 0,
    ...
    const filteredData = useMemo(() => {
        return allData.filter(item => {
            const matchesSearch = item.nombreEmpresa.toLowerCase().includes(searchTerm.toLowerCase()) ||
                item.rucDni.includes(searchTerm);
            const matchesTipoServicio = tipoServicioFilter === "TODOS" || item.tipoServicio === tipoServicioFilter;
            return matchesSearch && matchesTipoServicio;
        });
    }, [allData, searchTerm, tipoServicioFilter]);
    ...
    const handleTipoServicioFilter = (value: string) => {
        setTipoServicioFilter(value);
        setCurrentPage(1);
    };

    const openModal = () => setIsModalOpen(true);

    const handleSubmit = (servicio: any) => {
    ...
    // Función para obtener el color del badge de proceso
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-slate-600">Administra los servicios solicitados</p>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                        <div className="flex-1 max-w-md">
                            <div className="relative">
                                ...
                                <input
                                    placeholder="Buscar por empresa, RUC/DNI..."
                                    value={searchTerm}
                        ...
                        <div className="flex items-center space-x-3">
                            <select
                                ...
                                onChange={(e) => handleTipoServicioFilter(e.target.value)}
                                className="px-4 py-2.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors bg-white"
                            >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="border-b bg-slate-50 border-slate-200">
                            <tr>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Mes</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Empresa</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">RUC/DNI</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Tipo Servicio</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Proceso</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Técnico</th>
                                <th className="px-6 py-4 text-xs font-semibold tracking-wider text-center uppercase text-slate-600">Próximo Servicio</th>
                                ...
                                currentData.map((item) => (
                                    <tr key={item.id} className="transition-colors hover:bg-slate-50">
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                            <div className="text-sm font-medium text-slate-900">{item.mes}</div>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <div className="max-w-xs mx-auto">
                                                <p className="text-sm font-medium text-slate-900 line-clamp-2">{item.nombreEmpresa}</p>
                                                {item.telefono && <p className="text-xs text-slate-500">{item.telefono}</p>}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                            <div className="text-sm text-slate-900">{item.rucDni}</div>
                                        </td>
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getProcesoColor(item.proceso)}`}>
                                                {item.proceso}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <div className="text-sm text-slate-900">{item.tecnicoResponsable || "-"}</div>
                                        </td>
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                            <div className="text-sm text-slate-900">{item.fechaProximoServicio || "-"}</div>
                                        </td>
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
